...
impl<'a> Analyzed<'a> {
    ...
    fn production_enum(&self) -> Vec<Item> {
            ...
            let exprs = prod.body().iter().enumerate().map(|(i, sym)| {
                ...
                match sym {
                    SymbolicSymbol::Token(enriched_token) => {
                        ...
                        quote! {
                            let Some(semasia::parser::Symbol::Token(Token::#type_ident(#var_name))) = stacks.symbol_stack.pop() else { unreachable!("this is not a token") };
                    ...
                    SymbolicSymbol::NonTerminal(enriched_non_terminal) => {
                        ...
                        quote! {
                            let Some(semasia::parser::Symbol::NonTerminal(NonTerminal::#type_ident(#var_name))) = stacks.symbol_stack.pop() else { unreachable!("this is not a non terminal") };
                            stacks.state_stack.pop();
                        }
                    }
                }
        ...
        let file: syn::File = parse_quote! {
            ...
            impl semasia::parser::Reduce<NonTerminal, Token, __CompilerContext> for ProductionName {
                fn reduce(&self, ctx: &mut __CompilerContext, stacks: &mut semasia::parser::Stacks<NonTerminal, Token>) -> NonTerminal {
    ...
    fn const_tables(
            ...
            let row = row.into_iter().map::<syn::Expr, _>(|action| {
                match action.map::<syn::Expr, _>(|action| match action {
                    semasia_dyn_grammar::parsing::action::TokenAction::Shift(state) => {
                        parse_quote!(semasia::parser::TokenAction::Shift(#state))
                    }
                    semasia_dyn_grammar::parsing::action::TokenAction::Reduce(prod_id) => {
                        ...
                        parse_quote!(semasia::parser::TokenAction::Reduce(ProductionName::#prod_name))
                    }
                }) {
                ...
                match action.map::<syn::Expr, _>(|action| match action {
                    semasia_dyn_grammar::parsing::action::EofAction::Reduce(prod_id) => {
                        let actual_production = enriched_grammar
                            ...
                            .expect("production not found");
                        let prod_name = actual_production.id();
                        parse_quote!(semasia::parser::EofAction::Reduce(ProductionName::#prod_name))
                    }
                    semasia_dyn_grammar::parsing::action::EofAction::Accept => {
                        parse_quote!(semasia::parser::EofAction::Accept)
                    }
                }) {
                    Some(expr) => parse_quote!(Some(#expr)),
                    None => parse_quote!(None),
        ...
        let file: syn::File = parse_quote! {
            ...
            impl Tables {
                pub const TOKEN_TABLE: [[Option<semasia::parser::TokenAction<ProductionName>>; #token_count]; #state_count] = [
                    #(#token_actions,)*
                ];

                pub const EOF_TABLE: [Option<semasia::parser::EofAction<ProductionName>>; #state_count] = [
                    #(#eof_actions,)*
                ];
            ...
            impl semasia::parser::Tables<NonTerminal, Token, ProductionName> for Tables {
                fn query_token_table(current_state: usize, current_token: &Token) -> Option<semasia::parser::TokenAction<ProductionName>> {
                    Tables::TOKEN_TABLE[current_state][current_token.id()].clone()
                }
                fn query_eof_table(current_state: usize) -> Option<semasia::parser::EofAction<ProductionName>> {
        ...
        let token_table_patts = self
            ...
            .map(|(state, token_id, action)| {
                let action = match action {
                    semasia_dyn_grammar::parsing::action::TokenAction::Shift(state) => {
                        quote!(semasia::parser::TokenAction::Shift(#state))
                    }
                    semasia_dyn_grammar::parsing::action::TokenAction::Reduce(production) => {
                        let production = &self
                            ...
                            .extras()
                            .0;
                        quote!(semasia::parser::TokenAction::Reduce(ProductionName::#production))
                    }
                };
                        ...
                        let action = match action {
                            semasia_dyn_grammar::parsing::action::EofAction::Reduce(production) => {
                                let production = &self
                                    ...
                                    .extras()
                                    .0;
                                quote!(semasia::parser::EofAction::Reduce(ProductionName::#production))
                            }
                            semasia_dyn_grammar::parsing::action::EofAction::Accept => {
                                quote!(semasia::parser::EofAction::Accept)
                            }
                        };
    ...
    fn parser(&self) -> Item {
        let start_symbol = self.automaton.grammar().start_symbol().extras().id();
        parse_quote!(pub type Parser = semasia::parser::Parser<NonTerminal, Token, #start_symbol, ProductionName, Tables, __CompilerContext>;)
    }
}
